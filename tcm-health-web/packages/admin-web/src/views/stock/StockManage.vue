...
import { getAdminStockList, updateAdminStock, getAdminStockLogs } from 'shared/api/admin.js'
...
const loadStocks = async () => {
  try {
    const res = await getAdminStockList({ pageNum: pageNum.value, pageSize: pageSize.value, keyword: keyword.value, warningOnly: warningOnly.value || undefined })
...
const loadLogs = async () => {
  try {
    const res = await getAdminStockLogs({ pageNum: logPageNum.value, pageSize: logPageSize.value })
...
const submitEdit = async () => {
  try {
    await updateAdminStock(editRow.value.product_id, newStock.value, editRemark.value)
...
.stock-manage h3 { margin: 0 0 16px; }
