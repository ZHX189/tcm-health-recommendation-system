...
<template>
  <div class="health-record-page">
    ...
    <div class="page-header">
      ...
      <p class="page-desc">查看和管理所有用户的健康档案信息</p>
    </div>
...
import { getAdminHealthRecordList, deleteHealthRecord } from 'shared/api/admin.js'
...
async function fetchList() {
  ...
  try {
    ...
    const data = await getAdminHealthRecordList(params)
...
async function handleDelete(row) {
  try {
    await ElMessageBox.confirm(
      `确定要删除用户"${row.realName || row.userId}"的健康档案吗？`,
      '提示',
